				 ...
				 const struct nls_table *nls_cp)
{
	...
	*pbuffer = kmalloc(len, GFP_KERNEL);
	...
	if (ses->workstation_name == NULL) {
		len = cifs_strtoUTF16((__le16 *)tmp,
				      CIFS_DEFAULT_WORKSTATION_NAME,
				      CIFS_MAX_WORKSTATION_LEN, nls_cp);
		len *= 2;
		...
		len = cifs_strtoUTF16((__le16 *)tmp, ses->workstation_name,
				      CIFS_MAX_WORKSTATION_LEN, nls_cp);
